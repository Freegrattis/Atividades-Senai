...
#1
create view  viewSelecionaEspecieGato as
	SELECT id_animal,nome_animal,descricao_raca, nome_especie 
    FROM animal INNER JOIN raca ON animal.fk_raca=id_raca
    INNER JOIN especie ON raca.fk_especie=especie.id_especie 
    WHERE especie.nome_especie='Gato';
    
create view viewPesquisaAnimalDoCliente as
	SELECT nome_animal, nome_cliente 
    FROM animal LEFT JOIN cliente 
    ON animal.fk_cliente = cliente.id_cliente;
    
create view viewPesquisaGatoEShihtzu as
	SELECT AVG(peso_animal) FROM animal 
    INNER JOIN raca ON animal.fk_raca = raca.id_raca 
    INNER JOIN especie ON raca.fk_especie = especie.id_especie 
    WHERE especie.nome_especie = 'Cachorro';
    
create view viewPesquisaCachorro as
	SELECT AVG(peso_animal) FROM animal 
    INNER JOIN raca ON animal.fk_raca = raca.id_raca 
    INNER JOIN especie ON raca.fk_especie = especie.id_especie 
    WHERE especie.nome_especie = 'Cachorro';
    
create view viewSelecionaPorPesoEIdade as
	SELECT nome_animal, nome_cliente, telefone_principal_cliente, telefone_alternativo_cliente, email_cliente 
		FROM animal 
			INNER JOIN cliente ON animal.fk_cliente = cliente.id_cliente
            WHERE peso_animal >= 10 AND idade_animal >= 5;
            
#2
create user administrador;
create role papel_adiministracao;
...
#5
create role papel_funcionario;

#6
Grant select, insert, update
	on animal
    to papel_funcionario;
...
Grant all privileges
	on clinica_veterinaria
    to papel_adiministracao;

grant papel_adiministracao to administrador;

#11
REVOKE SELECT
ON funcao
FROM papel_funcionario;

#12
start transaction;
    create trigger trg_RemoverAnimaisClientes
...
#13
start transaction;
    insert into raca (descricao_raca, fk_especie) values ("Buldogue", 2);
commit;
CREATE TRIGGER trg_AdicionarAnimaisNulos
	AFTER INSERT ON raca
	FOR EACH ROW
		UPDATE animal
		SET fk_raca = (SELECT id_raca FROM raca WHERE descricao_raca = "Buldogue")
		WHERE fk_raca IS NULL;
